...
func testFileToManyThumbnails(t *testing.T) {
	...
	if err = randomize.Struct(seed, &b, thumbnailDBTypes, false, thumbnailColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, thumbnailDBTypes, false, thumbnailColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Thumbnails().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := FileSlice{&a}
	if err = a.L.LoadThumbnails(ctx, tx, false, (*[]*File)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.Thumbnails = nil
	if err = a.L.LoadThumbnails(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Thumbnails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testFileToManyAddOpThumbnails(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddThumbnails(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		count, err := a.Thumbnails().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testFileToOneRemoveOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	fileDBTypes = map[string]string{`ID`: `integer`, `Size`: `integer`, `Name`: `text`, `Alias`: `text`, `Hash`: `text`, `Slug`: `text`, `ContentType`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `UserID`: `integer`, `Private`: `boolean`, `Comment`: `text`}
	_           = bytes.MinRead
)
