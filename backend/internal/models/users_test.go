...
func testUserToManySetOpFiles(t *testing.T) {
	var err error
...
func testUserToManyRemoveOpFiles(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Name`: `text`, `Email`: `text`, `Provider`: `text`, `ProviderID`: `text`, `Role`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
