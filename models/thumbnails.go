...
var (
	thumbnailColumns               = []string{"id", "file_id", "width", "height", "hash", "created_at"}
...
// SetFileG of the thumbnail to the related item.
// Sets o.R.File to related.
// Adds o to related.R.Thumbnails.
// Uses the global database handle.
func (o *Thumbnail) SetFileG(insert bool, related *File) error {
	return o.SetFile(boil.GetDB(), insert, related)
}

// SetFileP of the thumbnail to the related item.
// Sets o.R.File to related.
// Adds o to related.R.Thumbnails.
// Panics on error.
func (o *Thumbnail) SetFileP(exec boil.Executor, insert bool, related *File) {
	if err := o.SetFile(exec, insert, related); err != nil {
		panic(boil.WrapErr(err))
	}
}

// SetFileGP of the thumbnail to the related item.
// Sets o.R.File to related.
// Adds o to related.R.Thumbnails.
// Uses the global database handle and panics on error.
func (o *Thumbnail) SetFileGP(insert bool, related *File) {
...
func FindThumbnailG(id int, selectCols ...string) (*Thumbnail, error) {
	return FindThumbnail(boil.GetDB(), id, selectCols...)
}

// FindThumbnailGP retrieves a single record by ID, and panics on error.
func FindThumbnailGP(id int, selectCols ...string) *Thumbnail {
...
func FindThumbnail(exec boil.Executor, id int, selectCols ...string) (*Thumbnail, error) {
...
func FindThumbnailP(exec boil.Executor, id int, selectCols ...string) *Thumbnail {
...
func (o *Thumbnail) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"thumbnails\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"thumbnails\" DEFAULT VALUES"
		}
...
func (o *Thumbnail) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"thumbnails\"", updateOnConflict, ret, update, conflict, insert)
...
func ThumbnailExists(exec boil.Executor, id int) (bool, error) {
	var exists bool
...
func ThumbnailExistsP(exec boil.Executor, id int) bool {
