...
func testThumbnailsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = thumbnail.Insert(tx, thumbnailColumnsWithoutDefault...); err != nil {
		t.Error(err)
	}
...
func testThumbnailToOneFileUsingFile(t *testing.T) {
	...
	if err = local.L.LoadFile(tx, false, (*[]*Thumbnail)(&slice)); err != nil {
		t.Fatal(err)
	}
	if local.R.File == nil {
...
var (
	thumbnailDBTypes = map[string]string{`CreatedAt`: `timestamp without time zone`, `FileID`: `integer`, `Hash`: `text`, `Height`: `integer`, `ID`: `integer`, `Width`: `integer`}
	_                = bytes.MinRead
)
